...
function move_to_gpu(array::CuArray{T,N}) where {T,N}
    return array
end
function move_to_gpu(array::Array{T,N}) where {T<:Union{Float32,Float64},N}
    return CuArray{Float32,N}(array)
end
function move_to_gpu(array::Array{T,N}) where {T<:Union{Int32,Int64},N}
    return CuArray{Int32,N}(array)
end
...
function estimate_plan_convergence(
    ...
    mu = move_to_gpu(measure.weights)                        # (n)
    nu = move_to_gpu(measure_l.weights)                      # (m)
    ...
    relative_errors_row = @. abs(marginals_row / mu - 1.0)   # (n)
    error_row = maximum(relative_errors_row)

    relative_errors_col = @. abs(marginals_col / nu - 1.0)   # (m)
